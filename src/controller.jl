...
mutable struct MPPIProblemCPU <: AbstractMPPIProblem
    ...
    u_upper::Vector{Float64} # upper control limit
    u_lower::Vector{Float64} # lower control limit
    ...
    function MPPIProblemCPU(
        ...
        u_upper=1e8*ones(model.dims.nu),
        u_lower=-1e8*ones(model.dims.nu),
        X_ref=nothing,
        U_md=nothing,
        ) 
        new(
...
mutable struct MPPIProblemGPU <: AbstractMPPIProblem
    model::AbstractDynamicsModel

    # simulation parameters
    tf::Float32 # final time
    tN::Int32 # number of time-discretization steps
    dt::Float32 # discretization step-size
    ...
    # boundary conditions
    x_init::Vector{Float32} # initial state 
    x_final::Vector{Float32} # terminal state

    X_ref # reference trajectory
    U_md # measured disturbance
    function MPPIProblemGPU(
        model;
        tf=model.tf,
        tN=model.tN,
        dt=model.dt,
...
function solve_mppi_cpu(
    ...
    if isnothing(δu)
        δu = rand(dist, tN, K) # sampling noise
    end
    ...
    Threads.@threads for k in 1:K
        ...
        for i in 1:tN
            ...
            uk = clamp.(uk, prob.u_lower, prob.u_upper) # clamp the value for control constraints
            t = (i - 1) * dt
            
            if isnothing(U_md)
                p = ODEParameter(params=params_arr, U_ref=uk)
            else
                p = ODEParameter(params=params_arr, U_ref=uk, U_md=prob.U_md[:,i])
            end
...
function solve_mppi_gpu(
    ...
    # CUDA.@sync begin
    #     @cuda threads=threads blocks=blocks kernel_compute_cost_gpu!(S,X_traj,U_traj,δu,æ_dim,hN,K,dt,N,Xf,ν)
    # end
    ...
    for k in 1:K
        ...
        for i in 1:tN
            ...
            X_arr[:, i+1] = X_arr[:, i] + rk4_step(dyn_funcs.f, X_arr[:, i], p, 0.0, dt) * dt
...
function compute_cost_cpu!(
    S::AbstractArray{Float64,1},
    x_init::AbstractArray{Float64,1},
    U::AbstractArray{Float64,2},
    δu::AbstractArray{Float64,2},
    dyn_funcs::MPPIDynamicsFunction,
    cost_funcs::MPPICostFunction,
    params_arr::AbstractArray{Float64,1},
    tN::Int64,
    dt::Float64,
    K::Int64,
    X_ref::AbstractArray{Float64,2},
    U_md=nothing,
)
    X_arr = zeros(size(x_init,1), tN+1) # state trajectory array
